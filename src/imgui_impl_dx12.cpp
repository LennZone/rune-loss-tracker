...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex)
{
    ...
    // Bail out before touching any ImGui texture state if the device is unhealthy.
    // This happens intermittently during startup (GPU TDR). ImGui will retry next frame.
    if (FAILED(bd->pd3dDevice->GetDeviceRemovedReason())) {
        OutputDebugStringA("RLT: UpdateTexture skipped — device in error state\n");
        return;
    }
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        if (bd->pTexUploadBuffer == nullptr || upload_size > bd->pTexUploadBufferSize)
        {
            ...
            if (FAILED(hr)) {
                char dbg[64]; sprintf_s(dbg, "RLT: TexUploadBuf failed hr=%08lX\n", (unsigned long)hr);
                OutputDebugStringA(dbg);
                // If we just ran the WantCreate block (need_barrier_before_copy == false),
                // roll back its side-effects so ImGui can retry cleanly next frame.
            ...
            if (FAILED(hr)) { SafeRelease(bd->pTexUploadBuffer); return; }
...
bool    ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the root signature
    {
        ...
        { OutputDebugStringA("RLT: D3D12SerializeRootSignature not found\n"); return false; }

        ID3DBlob* blob = nullptr;
        if (D3D12SerializeRootSignatureFn(&desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, nullptr) != S_OK)
        { OutputDebugStringA("RLT: SerializeRootSignature linear failed\n"); return false; }

        { HRESULT hr2 = bd->pd3dDevice->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&bd->pRootSignatureLinear)); char dbg[64]; sprintf_s(dbg, "RLT: RootSig linear hr=%08lX\n", hr2); OutputDebugStringA(dbg); }
        blob->Release();

        // Nearest sampler
        staticSampler[0].Filter = D3D12_FILTER_MIN_MAG_MIP_POINT;
        if (D3D12SerializeRootSignatureFn(&desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, nullptr) != S_OK)
        { OutputDebugStringA("RLT: SerializeRootSignature nearest failed\n"); return false; }

        { HRESULT hr2 = bd->pd3dDevice->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&bd->pRootSignatureNearest)); char dbg[64]; sprintf_s(dbg, "RLT: RootSig nearest hr=%08lX\n", hr2); OutputDebugStringA(dbg); }
        blob->Release();
    }
    ...
    // Create the vertex shader
    {
        ...
        { OutputDebugStringA("RLT: D3DCompile VS failed\n"); return false; }
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), nullptr, nullptr, nullptr, "main", "ps_5_0", 0, 0, &pixelShaderBlob, nullptr)))
        {
            ...
            OutputDebugStringA("RLT: D3DCompile PS failed\n"); return false;
        }
    ...
    { char dbg[80]; sprintf_s(dbg, "RLT: PSO linear hr=%08lX rootSig=%p\n", result_pipeline_state, bd->pRootSignatureLinear); OutputDebugStringA(dbg); }
    ...
    { char dbg[80]; sprintf_s(dbg, "RLT: PSO nearest hr=%08lX\n", result_pipeline_state); OutputDebugStringA(dbg); }
    vertexShaderBlob->Release();
    pixelShaderBlob->Release();
    if (result_pipeline_state != S_OK)
        return false;
...
void ImGui_ImplDX12_NewFrame()
{
    ...
    if (!bd->pPipelineStateLinear)
        ...
        { OutputDebugStringA("RLT: CreateDeviceObjects FAILED — overlay disabled\n"); return; }
}
